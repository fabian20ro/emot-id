...
- [x] **2.1** Unify modal architecture with shared shell
  `ResultModal.tsx`, `DontKnowModal.tsx`, `SessionHistory.tsx`, `SensationPicker.tsx`
  _6 hours_

- [x] **2.2** Remove raw z-index usage and enforce tokenized layering
  `DontKnowModal.tsx` (`z-50`), `App.tsx` hint overlay, `ModelBar.tsx` indicator
  _1 hour_

- [x] **2.3** Complete 44px touch-target audit for remaining actionable controls
...
- [x] **2.4** Improve modal accessibility semantics and explicit close affordances
  `DontKnowModal.tsx`, `SessionHistory.tsx`, `ResultModal.tsx`
  _2 hours_

- [x] **2.5** Make overlays and toasts safe-area aware on small devices
  `ResultModal.tsx`, `SessionHistory.tsx`, `SensationPicker.tsx`, `UndoToast.tsx`, `Onboarding.tsx`
  _2 hours_

- [x] **2.6** Prevent small-screen overflow in text-heavy dialogs
  `Onboarding.tsx`, `DontKnowModal.tsx`, `GuidedScan.tsx`
  _2 hours_

- [x] **2.7** Improve SessionHistory UX labels (not raw IDs)
...
- [x] **3.1** Fix guilt opposite action text
...
- [x] **3.2** Post-intervention effectiveness check
...
- [x] **3.3** Randomize bubble order on each render
...
- [x] **3.5** Model progression nudges in SessionHistory
...
- [x] **3.6** Document somatic signal weight provenance
...
- [x] **4.5** Hide DimensionalField axis labels after first use
  `DimensionalField.tsx` — on mobile, hide axis labels after user's first interaction (or first session); uses localStorage flag
  _1 hour_

- [x] **4.6** Evaluate constriction/tension merge
  `somatic/types.ts`, `somatic/data.json` — evaluated and kept separate for now; current evidence in-app still treats constriction as distinct from tension pending dedicated user-testing data.
  _6 hours (if merging)_

- [x] **4.7** Emotional granularity training mode
...
- [x] **5.1** Model selection on first launch
...
- [x] **5.2** Active vs. passive vocabulary tracking
...
- [x] **5.3** Validate somatic scoring against Nummenmaa maps
...
- [x] **5.4** Optional check-in reminders (PWA notifications)
...
- [x] Lazy loading models — `React.lazy` + `Suspense` for code splitting (somatic data.json is largest)
- [x] PWA offline indicator — show banner when using cached content (skip custom install prompt)

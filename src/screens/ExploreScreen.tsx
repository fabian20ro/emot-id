...
export function ExploreScreen({ onChoose, onPractice }: ExploreScreenProps) {
  ...
  const items = [
    { id: 'body' as const, label: t.body, Icon: Activity, color: 'var(--coral)', bg: 'var(--coral-soft)' },
    { id: 'affect' as const, label: t.affect, Icon: Crosshair, color: 'var(--mustard)', bg: 'var(--mustard-soft)' },
    { id: 'words' as const, label: t.words, Icon: ListTree, color: 'var(--blue)', bg: 'var(--blue-soft)' },
    { id: 'plutchik' as const, label: t.plutchik, Icon: Boxes, color: 'var(--teal)', bg: 'var(--teal-soft)' },
  ]
      ...
      <div className="route-grid">
        {items.map(({ id, label, Icon, color, bg }) => (
          <button type="button" className="route-card" key={id} data-testid={`explore-${id}`} onClick={() => onChoose(id)}>
            ...
            <span className="route-copy"><strong>{label}</strong><span>{t.learn}</span></span>
          </button>
        ))}
        <button type="button" className="route-card" onClick={onPractice}>
          ...
          <span className="route-copy"><strong>{t.practice}</strong><span>{t.learn}</span></span>
        </button>
      </div>
    </div>
  )

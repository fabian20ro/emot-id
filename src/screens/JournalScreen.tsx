...
export function JournalScreen({ sessions, loading, saveSessions, onOpenSession, onOpenChain }: JournalScreenProps) {
    ...
    <div className="screen" data-testid="journal-screen">
      ...
      {sessions.length > 0 && (
        <section aria-labelledby="patterns-title">
          ...
          {somatic.regionFrequencies.length > 0 && <div className="pattern-row"><span>{historyT.somaticTitle}</span><strong>{somatic.regionFrequencies.slice(0, 3).map((item) => `${item.regionId} (${item.count})`).join(', ')}</strong></div>}
        </section>
      )}
